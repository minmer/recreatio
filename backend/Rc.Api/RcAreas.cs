...
public static class RcAreas
{
        ...
        HttpContext ctx, RcDb db, RcMasterKey masterKeys, RcPermissions permissions, CreateAreaRequest body)
    {
        ...
        try
        {
            ...
            // der Titel liegt darunter (9.13). Deshalb entsteht der Schluessel
            // hier und wird unten an CutEpochAsync weitergereicht; erzeugte ihn
            // jede Seite fuer sich, waere der Titel unter einem Schluessel
            // versiegelt, den niemand bekommt.
                ...
                """, connection, tx))
            {
                ...
                // 7.8 — Die Kettenkennung ist der NAME der Kette, nicht ein
                // Verweis auf eine Zeile. Sie steht ab der Geburt fest, auch
                // solange noch kein Eintrag geschrieben wird.
            ...
            foreach (var capability in new[] { RcCapability.Admin, RcCapability.Certify })
            {
                ...
                await RcRoles.InsertCertificateAsync(connection, tx, certificate, certificate.Sign(ownerSign), ctx.RequestAborted);
            }

            var members = await RcAreaKeys.MembersAsync(connection, tx, areaId, ctx.RequestAborted);
            try
            {
                await RcAreaKeys.CutEpochAsync(connection, tx, areaId, RcAreaKeys.ReasonInitial,
                    members, ownerRoleId, epochKey, ctx.RequestAborted);
            }
            finally
            {
