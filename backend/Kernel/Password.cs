...
/// </summary>
public sealed record AccountSecrets
{
    ...
    /// </summary>
    public static AccountSecrets Create(Guid accountId, ReadOnlySpan<byte> passwordKey, ReadOnlySpan<byte> masterKey,
        ReadOnlySpan<byte> passwordSalt)
    {
        var loginSalt = Password.NewSalt();
        var aad = Aad.Create("kernel", "account", accountId, Field.AccountMasterKey, 1);

        return new AccountSecrets
        {
            PasswordSalt = passwordSalt.ToArray(),
            LoginSalt = loginSalt,
            LoginVerifier = Password.DeriveLoginVerifier(passwordKey, loginSalt),
            MasterKeySealed = Crypto.Seal(passwordKey, aad, masterKey)
        };
    }
    ...
    {
        var aad = Aad.Create("kernel", "account", accountId, Field.AccountMasterKey, 1);
        return Crypto.Open(passwordKey, aad, MasterKeySealed);
    }

    public AccountSecrets WithNewPassword(Guid accountId, ReadOnlySpan<byte> oldPasswordKey,
