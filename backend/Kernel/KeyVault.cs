...
public sealed class KeyVault : IDisposable
{
    ...
    /// ohne beides.
    /// </summary>
    public KeyVault(TimeSpan? idleTimeout = null, int maxEntries = 5_000, Func<DateTimeOffset>? clock = null)
    {
    ...
    /// </summary>
    public void Store(string sessionId, ReadOnlySpan<byte> openingPiece, KeyBundle bundle)
    {
        ...
        var aad = Aad.Create("kernel", "session", bundle.AccountId, Field.AccountMasterKey, 1);
        var plaintext = bundle.Serialize();
        try
        {
            var sealedBlob = Crypto.Seal(DeriveCacheKey(openingPiece, sessionId), aad, plaintext);
            var now = _clock();
    ...
    /// Betreiber.
    /// </summary>
    public KeyBundle Open(string sessionId, ReadOnlySpan<byte> openingPiece)
    {
        if (!_entries.TryGetValue(sessionId, out var entry))
            throw new UnlockRequiredException("Kein Schluesselbund fuer diese Sitzung.");

        var now = _clock();
        if (now - entry.LastUsedUtc > _idleTimeout)
        {
            Forget(sessionId);
            throw new UnlockRequiredException("Schluesselbund abgelaufen.");
        }

        var aad = Aad.Create("kernel", "session", entry.AccountId, Field.AccountMasterKey, 1);
        byte[] plaintext;
        try
        {
            plaintext = Crypto.Open(DeriveCacheKey(openingPiece, sessionId), aad, entry.Sealed);
        }
        catch (DecryptException)
        {
            ...
            throw new UnlockRequiredException("Schluesselbund liess sich nicht oeffnen.");
        }

        try
        {
            _entries[sessionId] = entry with { LastUsedUtc = now };
            return KeyBundle.Deserialize(plaintext);
        }
        finally
        {
            CryptographicOperations.ZeroMemory(plaintext);
    ...
    private static byte[] DeriveCacheKey(ReadOnlySpan<byte> openingPiece, string sessionId)
    {
        if (openingPiece.Length == 0)
            throw new UnlockRequiredException("Kein Oeffnungsstueck mitgeschickt.");
        return Crypto.Derive(openingPiece, Crypto.InfoCacheUnwrap(sessionId), Crypto.KeySize);
    }
...
public sealed class KeyBundle : IDisposable
{
    ...
    public KeyBundle(Guid accountId, byte[] masterKey, IReadOnlyDictionary<Guid, byte[]>? roleReadKeys = null)
    {
        if (masterKey.Length != Crypto.KeySize)
    ...
    public ReadOnlySpan<byte> RoleReadKey(Guid roleId)
    {
        ...
        {
            key = Crypto.DeriveRoleReadKey(_masterKey, roleId);
    ...
    internal byte[] Serialize()
    {
        ...
        var buf = new byte[16 + Crypto.KeySize + 4 + _roleReadKeys.Count * (16 + Crypto.KeySize)];
        AccountId.TryWriteBytes(buf.AsSpan(0, 16), bigEndian: true, out _);
        _masterKey.CopyTo(buf.AsSpan(16));
        BitConverter.TryWriteBytes(buf.AsSpan(16 + Crypto.KeySize, 4), _roleReadKeys.Count);

        var off = 16 + Crypto.KeySize + 4;
        foreach (var (roleId, key) in _roleReadKeys)
        {
            ...
            off += 16 + Crypto.KeySize;
        }
        return buf;
    }

    internal static KeyBundle Deserialize(ReadOnlySpan<byte> data)
    {
        var accountId = new Guid(data[..16], bigEndian: true);
        var masterKey = data.Slice(16, Crypto.KeySize).ToArray();
        var count = BitConverter.ToInt32(data.Slice(16 + Crypto.KeySize, 4));
        ...
        for (var i = 0; i < count; i++)
        {
            roles[new Guid(data.Slice(off, 16), bigEndian: true)] =
                data.Slice(off + 16, Crypto.KeySize).ToArray();
            off += 16 + Crypto.KeySize;
        }

        var bundle = new KeyBundle(accountId, masterKey, roles);
        CryptographicOperations.ZeroMemory(masterKey);
        foreach (var k in roles.Values) CryptographicOperations.ZeroMemory(k);
        return bundle;
    }

    private KeyBundle Guard() =>
        _disposed ? throw new ObjectDisposedException(nameof(KeyBundle),
...
/// </summary>
public sealed class UnlockRequiredException(string message) : Exception(message)
{
    public string Code => ErrorCodes.SessionUnlockRequired;
}
