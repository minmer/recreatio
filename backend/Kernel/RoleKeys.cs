...
public static class RoleKeys
{
    ...
    /// dauert.
    /// </summary>
    public static RoleIdentity Create(Guid roleId, ReadOnlySpan<byte> roleKey)
    {
        ...
        try
        {
            return new RoleIdentity
            {
                ...
                SignPrivateSealed = Crypto.Seal(roleKey, SignAad(roleId), signPrivate),
                WrapPrivateSealed = Crypto.Seal(roleKey, WrapAad(roleId), wrapPrivate),
                ...
                Fingerprint = Crypto.KeyIdFromPublicKey(signPublic),
                KeyVersion = 1
            };
        }
        finally
    ...
    public static byte[] PersonalRoleKey(ReadOnlySpan<byte> masterKey, Guid personalRoleId) =>
        Crypto.DeriveRoleReadKey(masterKey, personalRoleId);

    public static RSA OpenSignKey(RoleIdentity role, ReadOnlySpan<byte> roleKey) =>
        Import(Crypto.Open(roleKey, SignAad(role.RoleId), role.SignPrivateSealed));

    public static RSA OpenWrapKey(RoleIdentity role, ReadOnlySpan<byte> roleKey) =>
        Import(Crypto.Open(roleKey, WrapAad(role.RoleId), role.WrapPrivateSealed));
    ...
    public static byte[] GrantTo(byte[] holderWrapPublicKey, Guid grantedRoleId, ReadOnlySpan<byte> roleKey)
    {
        ...
        return Crypto.WrapKey(rsa, GrantAad(grantedRoleId), roleKey);
    }

    public static byte[] OpenGrant(RSA holderWrapPrivateKey, Guid grantedRoleId, ReadOnlySpan<byte> sealedBlob) =>
        Crypto.UnwrapKey(holderWrapPrivateKey, GrantAad(grantedRoleId), sealedBlob);

    /// <summary>Ein frischer RoleKey fuer eine Rolle, die mehr als ein Halter haben kann.</summary>
    public static byte[] NewRoleKey() => Crypto.NewSymmetricKey();

    private static Aad SignAad(Guid roleId) =>
        Aad.Create("kernel", "role", roleId, Field.RoleSignPrivate, 1);

    private static Aad WrapAad(Guid roleId) =>
        Aad.Create("kernel", "role", roleId, Field.RoleWrapPrivate, 1);
    ...
    /// </summary>
    private static Aad GrantAad(Guid grantedRoleId) =>
        Aad.Create("kernel", "role_grant", grantedRoleId, Field.RoleWrapPrivate, 1);
...
public sealed record RoleIdentity
{
