...
/// 21.9 liegen als ausfuehrbare Pruefaelle in Kernel.Tests; sie sind
...
public static class Crypto
{
    ...
    public static string InfoRoleRead(Guid roleId) => $"recreatio:v1:role-read:{Ids.ToText(roleId)}";
    public static string InfoKeyId => "recreatio:v1:keyid";
    public static string InfoSharedView(Guid viewId) => $"recreatio:v1:shared-view:{Ids.ToText(viewId)}";
    ...
    public static byte[] BuildHeader(Alg alg, ReadOnlySpan<byte> keyId)
    {
    ...
    /// </summary>
    private static byte[] FullAad(ReadOnlySpan<byte> header, Aad aad)
    {
        var tail = aad.ToUtf8();
    ...
    /// </summary>
    public static byte[] Seal(ReadOnlySpan<byte> key, Aad aad, ReadOnlySpan<byte> plaintext)
    {
        var nonce = RandomNumberGenerator.GetBytes(NonceSize);
        return SealWithNonce(key, aad, plaintext, nonce);
    }

    public static byte[] Seal(ReadOnlySpan<byte> key, Aad aad, string plaintext)
        => Seal(key, aad, Encoding.UTF8.GetBytes(plaintext));

    /// <summary>Nur fuer Testvektoren. Produktivcode nutzt <see cref="Seal(ReadOnlySpan{byte}, Aad, ReadOnlySpan{byte})"/>.</summary>
    internal static byte[] SealWithNonce(ReadOnlySpan<byte> key, Aad aad, ReadOnlySpan<byte> plaintext, ReadOnlySpan<byte> nonce)
    {
        ...
        var header = BuildHeader(Alg.AesGcm256, KeyId(key));
    ...
    /// <summary>
    /// Wirft <see cref="DecryptException"/>, wenn die AAD nicht passt — das ist
    /// der Zweck der Konvention und nach 15.9 ein Sicherheitsereignis, das
    /// gezaehlt werden MUSS.
    /// </summary>
    public static byte[] Open(ReadOnlySpan<byte> key, Aad aad, ReadOnlySpan<byte> blob)
    {
        if (key.Length != KeySize) throw new ArgumentException("Schluessel muss 32 Byte lang sein.", nameof(key));

        var header = ReadHeader(blob);
        if (header.Alg != Alg.AesGcm256)
            throw new DecryptException(DecryptError.UnknownAlgorithm, $"AlgId 0x{(byte)header.Alg:x2} ist hier nicht erwartet.");
        if (blob.Length < HeaderSize + NonceSize + TagSize)
            throw new DecryptException(DecryptError.Malformed, $"Huelle zu kurz: {blob.Length} Byte.");
        ...
        catch (CryptographicException)
        {
            ...
            // Eingrenzung hilft.
            throw new DecryptException(DecryptError.AadMismatch,
                $"Integritaetspruefung fehlgeschlagen fuer AAD '{aad.Text}'.");
        }
        return pt;
    }

    public static string OpenText(ReadOnlySpan<byte> key, Aad aad, ReadOnlySpan<byte> blob)
        => Encoding.UTF8.GetString(Open(key, aad, blob));

    // ---- Kopf lesen -----------------------------------------------------------

    public readonly record struct Header(byte Version, Alg Alg, byte[] KeyId);
    ...
    /// </summary>
    public static Header ReadHeader(ReadOnlySpan<byte> blob)
    {
        if (blob.Length < HeaderSize)
            throw new DecryptException(DecryptError.Malformed, "Huelle kuerzer als der Kopf.");
        if (blob[0] != Magic[0] || blob[1] != Magic[1])
            throw new DecryptException(DecryptError.Malformed, "Kein Recreatio-Blob (Magic fehlt).");
        if (blob[2] != FormatVersion)
            throw new DecryptException(DecryptError.UnknownFormat, $"Formatversion {blob[2]} ist unbekannt.");

        var alg = (Alg)blob[3];
        if (alg is not (Alg.AesGcm256 or Alg.RsaOaep4096))
            throw new DecryptException(DecryptError.UnknownAlgorithm, $"AlgId 0x{blob[3]:x2} ist unbekannt.");

        return new Header(blob[2], alg, blob.Slice(4, 16).ToArray());
    }
    ...
    /// </summary>
    public static byte[] WrapKey(RSA wrapPublicKey, Aad aad, ReadOnlySpan<byte> keyToWrap)
    {
        var keyId = KeyIdFromPublicKey(wrapPublicKey.ExportSubjectPublicKeyInfo());
        var header = BuildHeader(Alg.RsaOaep4096, keyId);
    ...
    public static byte[] UnwrapKey(RSA wrapPrivateKey, Aad aad, ReadOnlySpan<byte> blob)
    {
        var header = ReadHeader(blob);
        if (header.Alg != Alg.RsaOaep4096)
            throw new DecryptException(DecryptError.UnknownAlgorithm, "Erwartet wurde eine RSA-verpackte Huelle.");
        ...
        {
            throw new DecryptException(DecryptError.WrongKey, "Verpackung liess sich nicht oeffnen.");
        }
...
/// </summary>
public sealed class DecryptException(DecryptError error, string message) : Exception(message)
{
    public DecryptError Error { get; } = error;

    /// <summary>Fehlerkennung nach 15.7, hierarchisch: &lt;bereich&gt;.&lt;fall&gt;.</summary>
    public string Code => Error switch
    {
        DecryptError.MissingEpoch     => "crypto.missing_epoch",
        DecryptError.MissingKey       => "crypto.missing_key",
        DecryptError.AadMismatch      => "crypto.aad_mismatch",
        DecryptError.Malformed        => "crypto.malformed",
        DecryptError.UnknownFormat    => "crypto.unknown_format",
        DecryptError.UnknownAlgorithm => "crypto.unknown_algorithm",
        DecryptError.WrongKey         => "crypto.wrong_key",
        _ => "crypto.failed"
    };
}
