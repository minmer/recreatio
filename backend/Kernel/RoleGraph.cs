...
/// Schluesselerreichbarkeit ist (siehe <see cref="RoleKeys"/>), bedeutet ein
...
/// geht.
/// </summary>
public sealed class RoleCycleException(Guid from, Guid to)
    : Exception($"Die Kante {Ids.ToText(from)} -> {Ids.ToText(to)} wuerde einen Kreis schliessen.")
{
    public Guid From { get; } = from;
    public Guid To { get; } = to;
    public string Code => ErrorCodes.RoleCycle;
}
