...
app.MapGet("/account/profile", async (HttpContext context, RecreatioDbContext dbContext, CancellationToken ct) =>
{
    if (!TryGetUserId(context, out var userId))
    {
        return Results.Unauthorized();
...
app.MapPost("/account/profile", async (ProfileUpdateRequest request, HttpContext context, RecreatioDbContext dbContext, CancellationToken ct) =>
{
    if (!TryGetUserId(context, out var userId))
    {
        return Results.Unauthorized();
