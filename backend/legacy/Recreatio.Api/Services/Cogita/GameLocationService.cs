...
public sealed class GameLocationService : IGameLocationService
{
        ...
        CancellationToken ct)
    {
        ...
        double? roundedLat = null;
        double? roundedLon = null;
        if (priorLocation.HasValue)
        {
            roundedLat = Math.Round(priorLocation.Value.Latitude, 3, MidpointRounding.AwayFromZero);
            roundedLon = Math.Round(priorLocation.Value.Longitude, 3, MidpointRounding.AwayFromZero);
        }
