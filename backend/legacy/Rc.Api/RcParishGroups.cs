...
///
/// <para>
/// <b>WAS HIER ABSEHBAR NICHT MEHR STIMMT — eine Pfarrei ist nicht die
/// einzige Organisation.</b>
/// </para>
///
/// <para>
/// Die Pfarrseite ist in Wahrheit die Seite einer ORGANISATION; eine Pfarrei
/// ist eine Art davon. Und eine Gruppe gehoert nicht zwingend zu genau einer:
/// die Oase in der Pfarrei gehoert AUCH zur ueberpfarrlichen Bewegung, die
/// Caritasgruppe auch zur Caritas. Die Beziehung ist n:m, mit vermutlich einer
/// „Heimat", der die Adresse gehoert, und weiteren, die die Gruppe nur
/// auffuehren.
/// </para>
///
/// <para>
/// <c>rc_parish_group</c> kann das heute nicht: <c>parish_id</c> ist NOT NULL
/// mit einem einzelnen Fremdschluessel, und der Namensraum der Adresse ist
/// <c>UNIQUE (parish_id, slug)</c> — eine Pfarrei, genau eine, fuer immer.
/// </para>
///
/// <para>
/// <b>Die teure Haelfte ist aber schon frei.</b> Was eine Gruppe AUSMACHT —
/// Bereich, Epochenschluessel, Kalender, Mitgliedsrolle, Amt — weiss von
/// Pfarreien nichts. Nur diese Zeile und die Adresse binden sie an eine. Der
/// Umbau ist also eine Verbindungstabelle und eine Route, keine Neuverschluesselung.
/// Bis dahin gilt: KEINE weiteren pfarreifoermigen Spalten an diese Tabelle.
/// </para>
///
/// <para>
/// Ein Vorbild gibt es schon, und es waere zweimal beantwortet, wenn man hier
/// etwas anderes erfaende: <c>rc_0026</c> hat den Veranstalter einer
/// Veranstaltung zu einer ROLLE beliebiger Art gemacht — Person, Pfarrei oder
/// Gemeinschaft —, die Art traegt <c>rc_role.kind</c>. Genau damit der
/// Veranstalter keine Pfarrei sein MUSS. Pfarrei und Gruppe sind dem nicht
/// gefolgt.
/// </para>
///
