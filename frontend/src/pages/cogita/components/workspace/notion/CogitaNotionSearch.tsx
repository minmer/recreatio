...
export function CogitaNotionSearch({
  copy,
  authLabel,
  showProfileMenu,
  onProfileNavigate,
  ...
  const bulkDeleteCopy = useMemo(() => {
    if (language === 'de') {
      return {
        confirm: 'Ausgewählte Notionen löschen? Dieser Vorgang kann nicht rückgängig gemacht werden.',
        allFailed: 'Ausgewählte Notionen konnten nicht gelöscht werden. Entferne zuerst Verknüpfungen und Verwendungen in Sammlungen/Verbindungen.',
        partialFailed: 'Einige ausgewählte Notionen konnten nicht gelöscht werden. Entferne zuerst Verknüpfungen und Verwendungen in Sammlungen/Verbindungen.'
      };
    }
    if (language === 'en') {
      return {
        confirm: 'Delete selected notions? This cannot be undone.',
        allFailed: 'Failed to delete selected notions. Remove links and collection/connection usage first.',
        partialFailed: 'Some selected notions could not be deleted. Remove links and collection/connection usage first.'
      };
    }
    return {
  ...
  const searchFiltersPayload = useMemo(() => {
    const payload: Record<string, string> = { sourceKind: 'info' };
    if (hasActiveTypeFilters) {
      for (const field of typeFilterConfig) {
        const value = (typeFilters[field.key] ?? '').trim();
        if (!value) continue;
        payload[field.path ?? field.key] = value;
      }
    }
    const referenceId = (typeFilters.__referenceId ?? '').trim();
    if (referenceId) {
      payload['link.references'] = referenceId;
    }
    return payload;
  }, [hasActiveTypeFilters, typeFilterConfig, typeFilters]);
  ...
  }, [collectionScopeReady, filterCollectionId]);
  ...
  const visibleSearchResults = useMemo(
    () => visibleResults.map((info) => ({ info, cards: [] })),
    [visibleResults]
  );
  ...
  const openInfo = (infoId: string) => {
    navigate(`/cogita/workspace/libraries/${libraryId}/notions/${encodeURIComponent(infoId)}`, { replace: true });
  };
  ...
  const editSelectedInfo = () => {
    ...
    navigate(`/cogita/workspace/libraries/${libraryId}/notions/${encodeURIComponent(selectedInfoIdFromRoute)}/edit`, { replace: true });
  };
...
function getInfoOverviewCopy(language: 'pl' | 'en' | 'de') {
  ...
  return {
    ...
    loadError: 'Failed to load notion.',
    knownness: 'Knownness',
    reviews: 'Reviews',
    ...
    linkedCardsHint: 'Cards depending on this notion',
...
export function CogitaNotionSearchList({
  libraryId,
  infoType,
  query,
  onQueryChange,
  ...
  resultsOverride?: CogitaNotionSearchResult[];
  allowSelection?: boolean;
  allowMultiSelect?: boolean;
  selectedIds?: string[];
  onSelectionChange?: (ids: string[]) => void;
  onNotionToggleSelection?: (result: CogitaNotionSearchResult, checked: boolean) => void;
  onNotionSelect?: (result: CogitaNotionSearchResult) => void;
  onNotionOpen?: (result: CogitaNotionSearchResult) => void;
  ...
  renderTypeLabel?: (result: CogitaNotionSearchResult) => string;
  showStatusMessages?: boolean;
  disabled?: boolean;
  onStatusChange?: (status: 'idle' | 'loading' | 'ready' | 'error') => void;
  onResultsChange?: (results: CogitaNotionSearchResult[]) => void;
}) {
  const [localQuery, setLocalQuery] = useState('');
  const [localSelectedIds, setLocalSelectedIds] = useState<string[]>([]);
  const [results, setResults] = useState<CogitaNotionSearchResult[]>([]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (cancelled) return;
        const filtered = enriched.filter((item): item is CogitaNotionSearchResult => Boolean(item));
        setResults(filtered);
  ...
  const handleSelectionToggle = (result: CogitaNotionSearchResult, checked: boolean) => {
    ...
    onNotionToggleSelection?.(result, checked);
  };
  const handleResultSelect = (result: CogitaNotionSearchResult) => {
    if (onNotionSelect) {
      onNotionSelect(result);
      return;
    }
    onNotionOpen?.(result);
  };
  const handleResultOpen = (result: CogitaNotionSearchResult) => {
    onNotionOpen?.(result);
  };
