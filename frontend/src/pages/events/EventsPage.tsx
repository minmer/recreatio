...
const START_PATTERN_BANDS: Array<{ id: string; x: number; y: number; scale: number }> = [
  { id: 'b1', x: -22, y: 10, scale: 7.35 },
  { id: 'b2', x: 8, y: 740, scale: 7.28 },
  { id: 'b3', x: -26, y: 1470, scale: 7.32 },
  { id: 'b4', x: 10, y: 2200, scale: 7.3 },
  { id: 'b5', x: -20, y: 2930, scale: 7.34 },
  { id: 'b6', x: 6, y: 3660, scale: 7.29 },
  { id: 'b7', x: -24, y: 4390, scale: 7.36 }
];
      ...
      {START_PATTERN_BANDS.map((band) => (
        <g key={band.id} transform={`translate(${band.x} ${band.y}) scale(${band.scale})`}>
          {BLACK_STONES_SUBPATHS.map((subpath, index) => (
            <path key={`${band.id}-${index}`} d={subpath} fill={START_MOSAIC_COLORS[index % START_MOSAIC_COLORS.length]} />
          ))}
        </g>
      ))}
      {stones.map((stone) => (
