...
const BLACK_STONE_SUBPATHS = BLACK_STONES_PATTERN_PATH.match(/[Mm][^Mm]*/g) ?? [];
const BLACK_STONE_PART_COLORS = ['#F45741', '#FB9926', '#88C529'] as const;
const BLACK_STONE_GROUP_SIZE = Math.max(1, Math.ceil(BLACK_STONE_SUBPATHS.length / BLACK_STONE_PART_COLORS.length));
  ...
  return (
    ...
    >
      <g transform="translate(0,4.000004)">
        ...
        {BLACK_STONE_SUBPATHS.map((pathData, index) => (
          <path
            key={`stone-${index}`}
            fill={BLACK_STONE_PART_COLORS[Math.min(BLACK_STONE_PART_COLORS.length - 1, Math.floor(index / BLACK_STONE_GROUP_SIZE))]}
            d={pathData}
          />
        ))}
      </g>
