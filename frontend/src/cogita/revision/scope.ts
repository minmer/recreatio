type InfoSelectionRevisionSeed = {
  kind: 'info-selection';
  libraryId: string;
  infoIds: string[];
  createdAt: string;
...
const STORAGE_PREFIX = 'cogita.revision.seed:';

function storageKey(seedId: string) {
  return `${STORAGE_PREFIX}${seedId}`;
}

export function saveInfoSelectionRevisionSeed(libraryId: string, infoIds: string[]) {
  ...
  const payload: InfoSelectionRevisionSeed = {
    kind: 'info-selection',
    libraryId,
    infoIds: uniqueIds,
...
export function loadInfoSelectionRevisionSeed(libraryId: string, seedId: string) {
  if (typeof window === 'undefined') return null;
  const raw = window.localStorage.getItem(storageKey(seedId));
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as Partial<InfoSelectionRevisionSeed>;
