...
const STORAGE_PREFIX = 'cogita.dependency.seed:';

function storageKey(seedId: string) {
  return `${STORAGE_PREFIX}${seedId}`;
}

export function saveDependencySelectionSeed(libraryId: string, infoIds: string[]) {
  ...
  const payload: DependencySelectionSeed = {
    kind: 'info-selection',
    libraryId,
    infoIds: uniqueIds,
...
export function loadDependencySelectionSeed(libraryId: string, seedId: string) {
  if (typeof window === 'undefined') return null;
  const raw = window.localStorage.getItem(storageKey(seedId));
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as Partial<DependencySelectionSeed>;
