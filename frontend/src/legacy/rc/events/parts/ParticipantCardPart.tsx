...
import { getParticipantCard, saveParticipantCard, type EventParticipantCard } from './rcPartsApi';
import {
  CARD_LEVELS,
  LEVEL_OPTIONS,
  readLevel,
...
export const participantCardPart = definePart<CardConfig>({
  ...
  Renderer: ({ config, ctx }) => {
    ...
    const partId = ctx.part.partId;
    const intakeKey = ctx.intakePublicKey;
    const spec = CARD_LEVELS[config.level];
    ...
    const load = useCallback(async () => {
      ...
      try {
        /*
          EINE ABGEGEBENE KARTE KOMMT NICHT ZURUECK.

          Sie liegt versiegelt, und den Schluessel hat die Veranstaltung — nicht
          der, der sie abgegeben hat. Das Formular faengt deshalb leer an. Es
          mit erfundenen Werten zu fuellen waere schlimmer als leer: es saehe
          aus, als stuende das schon so in der Karte.
        */
    ...
    const save = async (event: FormEvent) => {
      ...
      setPending(true);
      try {
        await saveParticipantCard(token, partId, {
          data: payload,
        ...
        /*
          Nach dem Abgeben ist die Karte fort — versiegelt und nicht mehr zu
          lesen. Was bleibt, ist die Bestaetigung; `saved` traegt sie.
        */
        setCard(null);
        setSaved(true);
