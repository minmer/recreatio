...
export const galleryPart = definePart<GalleryConfig>({
  ...
  Renderer: ({ config, ctx }) => (
    <Gallery config={config} slug={ctx.siteSlug} partId={ctx.part.partId} token={ctx.claim} />
  ),

  Editor: ({ config, onChange, ctx }) => <GalleryEditor config={config} onChange={onChange} partId={ctx.part.partId} />
});
