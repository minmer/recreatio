...
export const RC_PARTS = {
  ...
  /*
   * DER ARBEITSPLATZ — die Eingangshalle hinter der Anmeldung.
   *
   * <b>Was er ist.</b> Alles, was die oeffentlichen Seiten NICHT sind: was
   * dieser Mensch hier zu tun hat. Seine Organisationen, seine Gruppen, seine
   * Termine. Eine oeffentliche Seite gehoert einer Pfarrei; der Arbeitsplatz
   * gehoert dem, der davorsitzt.
   *
   * <b>Warum er `slugged: false` ist.</b> Es gibt nur EINEN — deinen. Ein Name
   * dahinter waere der Arbeitsplatz eines anderen, und den gibt es nicht zu
   * sehen. Was danach im Pfad steht, sind Ansichten desselben Platzes
   * (`…/workspace/groups`), keine fremden Plaetze.
   *
   * <b>Er HIESS `workshop`.</b> Umbenannt, weil er nie etwas gezeigt hat und
   * genau ein Verweis darauf zeigte — der Anmeldeknopf im Veranstaltungs-
   * katalog, der damit in einer leeren Huelle landete. Zwei fast gleiche
   * Woerter (`workshop`/`workspace`) nebeneinander im Register waeren die
   * naechste Verwechslung gewesen.
   */
