import type { RouteKey } from '../types/navigation';

export type Copy = {
  nav: {
    home: string;
    parish: string;
    events: string;
    limanowa: string;
    cogita: string;
    chat: string;
    calendar: string;
    faq: string;
    legal: string;
    login: string;
    account: string;
  };
  accountMenu: {
    profile: string;
    secureModeOn: string;
    secureModeOff: string;
    logout: string;
  };
  hero: {
    headline: string;
    subtitle: string;
    ctaPrimary: string;
    ctaSecondary: string;
  };
  modules: {
    title: string;
    items: Array<{
      title: string;
      desc: string;
      tag: string;
      route: RouteKey;
    }>;
  };
  access: {
    title: string;
    subtitle: string;
    login: string;
    register: string;
    loginId: string;
    password: string;
    confirm: string;
    displayName: string;
    secureMode: string;
    rememberMe: string;
    create: string;
    signIn: string;
    statusTitle: string;
    statusReady: string;
    loginRequired: string;
    loginError: string;
    registerError: string;
    loginTaken: string;
    passwordMismatch: string;
    passwordTooShort: string;
    passwordCommon: string;
    passwordWeak: string;
    passwordStrong: string;
    loadingLogin: string;
    loadingRegister: string;
    sessionLabel: string;
    checkSession: string;
    toggleMode: string;
    logout: string;
  };
  dashboard: {
    title: string;
    subtitle: string;
    cards: Array<{ title: string; desc: string; action: string; route?: RouteKey }>;
  };
  parish: {
    title: string;
    subtitle: string;
    items: string[];
    note: string;
    loginCta: string;
  };
  events: {
    slideTitle: string;
    slideSubtitle: string;
    title: string;
    subtitle: string;
    chooserTitle: string;
    chooserSubtitle: string;
    pagesLabel: string;
    openEvent: string;
    backToEvents: string;
    unknownEvent: string;
    unknownPage: string;
    kal26Header: {
      information: string;
      plan: string;
      register: string;
      faq: string;
      history: string;
      gallery: string;
      contact: string;
      organizer: string;
    };
  };
  limanowa: {
    slideTitle: string;
    slideSubtitle: string;
    title: string;
    subtitle: string;
    inBuildTitle: string;
    inBuildText: string;
  };
  chat: {
    slideTitle: string;
    slideSubtitle: string;
    title: string;
    subtitle: string;
    loginCta: string;
  };
  calendar: {
    slideTitle: string;
    slideSubtitle: string;
    title: string;
    subtitle: string;
    loginCta: string;
    roadmapTitle: string;
    examplesTitle: string;
    note: string;
    items: string[];
    examples: string[];
  };
  cogita: {
    title: string;
    subtitle: string;
    items: string[];
    note: string;
    loginCta: string;
    shell: {
      back: string;
      up: string;
      changeRole: string;
    };
    roleIntro: {
      kicker: string;
      title: string;
      roles: {
        student: { title: string; subtitle: string };
        teacher: { title: string; subtitle: string };
        explore: { title: string; subtitle: string };
      };
    };
    libraryTemplates: {
      title: string;
      subtitle: string;
      nameLabel: string;
      namePlaceholder: string;
      create: string;
      creating: string;
      back: string;
      nameRequired: string;
      createFailed: string;
      newLibraryButton: string;
      templates: {
        vocabulary:   { title: string; desc: string };
        knowledgeMap: { title: string; desc: string };
        course:       { title: string; desc: string };
        research:     { title: string; desc: string };
        flashcards:   { title: string; desc: string };
        custom:       { title: string; desc: string };
      };
    };
    libraryPicker: {
      kicker: string;
      title: string;
      done: string;
      skip: string;
      tabs: {
        own: string;
        public: string;
        linked: string;
      };
      status: {
        open: string;
        hidden: string;
        removed: string;
      };
      consequences: {
        ownOpen: string;
        ownHidden: string;
        ownRemoved: string;
        otherRemoved: string;
      };
      link: {
        placeholder: string;
        add: string;
        loading: string;
        error: string;
        notFound: string;
        alreadyAdded: string;
      };
      empty: {
        own: string;
        public: string;
        linked: string;
      };
      publicNote: string;
      confirm: {
        title: string;
        body: string;
        ownNote: string;
        linkedNote: string;
        action: string;
        cancel: string;
      };
    };
    user: {
      librariesKicker: string;
      librariesTitle: string;
      librariesSubtitle: string;
      createLibraryTitle: string;
      libraryNameLabel: string;
      libraryNamePlaceholder: string;
      createLibraryAction: string;
      availableLibraries: string;
      loadingLibraries: string;
      noLibraries: string;
      libraryRoleLabel: string;
      openOverview: string;
      openCards: string;
      libraryNameRequired: string;
      libraryCreateFailed: string;
    };
    workspace: {
      navigationAria: string;
      layers: {
        library: string;
        target: string;
        collection: string;
        revision: string;
      };
      noLibraryOption: string;
      selectCollectionOption: string;
      targets: {
        libraryOverview: string;
        allCards: string;
        newCard: string;
        allCollections: string;
        allRevisions: string;
        newCollection: string;
        transfer: string;
        storyboards: string;
        newStoryboard: string;
        texts: string;
        newText: string;
        games: string;
        collectionRevision: string;
        sharedRevisions: string;
        dependencies: string;
      };
      revisions: {
        detail: string;
        graph: string;
        settings: string;
        run: string;
        live: string;
      };
      status: {
        loadFailed: string;
        savePrefsFailed: string;
        selectLibraryFirst: string;
        loadingCollections: string;
        noCollections: string;
        loadingRevisions: string;
        noRevisions: string;
      };
      panels: {
        currentPosition: string;
        quickCreate: string;
        collections: string;
      };
      path: {
        noLibrarySelected: string;
        noCollectionSelected: string;
        noCollectionLayer: string;
        currentRoute: string;
      };
      links: {
        libraryOverview: string;
        allCards: string;
        sharedRevisions: string;
      };
      cards: {
        collectionType: string;
        itemsSuffix: string;
      };
      revisionForm: {
        nameLabel: string;
        namePlaceholder: string;
        createAction: string;
        saveAction: string;
      };
      infoActions: {
        overview: string;
        edit: string;
        seeCards: string;
      };
      infoMode: {
        search: string;
        create: string;
      };
      sidebar: {
        title: string;
        openMenu: string;
        closeMenu: string;
        currentPath: string;
        explore: string;
        context: string;
        contextEmpty: string;
        alwaysAvailable: string;
        libraryActionsHint: string;
        infoActionsHint: string;
        selectedInfoActionsHint: string;
        collectionActionsHint: string;
        revisionParticipantSessionsHint: string;
        revisionHostedLiveSessionsHint: string;
        noRevisionHostedLiveSessions: string;
        revisionLiveSessionsHint: string;
        noRevisionLiveSessions: string;
        accountSettings: string;
        cogitaHome: string;
      };
    };
    core: {
      home: {
        title: string;
        subtitle: string;
        libraryIdLabel: string;
        runIdLabel: string;
        idPlaceholder: string;
        startNewRunAction: string;
        openRunAction: string;
      };
      run: {
        kicker: string;
        defaultRunTitle: string;
        generatedRunTitle: string;
        defaultParticipantName: string;
        scopeLabel: string;
        statusLabel: string;
        participantLabel: string;
        returnToRecreatio: string;
        backToCogita: string;
        loading: string;
        initFailed: string;
        submitFailed: string;
        nextFailed: string;
        currentPrompt: string;
        answerLabel: string;
        answerPlaceholder: string;
        correctAction: string;
        wrongAction: string;
        blankTimeoutAction: string;
        noCardSelected: string;
        selectNextCardAction: string;
        revealTitle: string;
        correctAnswerLabel: string;
        participantAnswerLabel: string;
        distributionLabel: string;
        scoreFactorsLabel: string;
        totalPointsLabel: string;
        nextCardAction: string;
        statsTitle: string;
        attemptsLabel: string;
        correctLabel: string;
        wrongLabel: string;
        blankLabel: string;
        knownessLabel: string;
        pointsLabel: string;
      };
    };
    introSlides: Array<{
      id: string;
      title: string;
      subtitle?: string;
      body?: string;
      micro?: string;
      cta: string;
      secondary?: string;
    }>;
    page: {
      nav: Array<{ id: string; label: string }>;
      hero: {
        tag: string;
        title: string;
        subtitle: string;
        primaryCta: string;
        secondaryCta: string;
      };
      homeSlides: Array<{
        id: string;
        title: string;
        text: string;
        ctaLabel: string;
        variant?: 'secondary';
      }>;
      stats: Array<{ value: string; label: string }>;
      highlights: Array<{ title: string; desc: string }>;
      sections: Array<{
        id: string;
        tag: string;
        title: string;
        subtitle: string;
        cards: Array<{ title: string; desc: string; meta?: string }>;
        bullets: string[];
      }>;
      join: {
        title: string;
        subtitle: string;
        cards: Array<{ title: string; desc: string; action: string }>;
      };
    };
    library: {
      nav: {
        overview: string;
        list: string;
        add: string;
        collections: string;
      };
      navLabel: string;
      actions: {
        backToCogita: string;
        libraryOverview: string;
        openList: string;
        collections: string;
        addInfo: string;
        createCollection: string;
        saveCollection: string;
        startRevision: string;
        collectionDetail: string;
      };
      infoTypes: {
        any: string;
        vocab: string;
        language: string;
        word: string;
        sentence: string;
        topic: string;
        collection: string;
        person: string;
        institution: string;
        collective: string;
        orcid: string;
        address: string;
        email: string;
        phone: string;
        media: string;
        work: string;
        geo: string;
        musicPiece: string;
        musicFragment: string;
        source: string;
        question: string;
        quote: string;
        computed: string;
        python: string;
      };
      connectionTypes: {
        wordLanguage: string;
        citationLanguage: string;
        wordTopic: string;
        languageSentence: string;
        translation: string;
        reference: string;
        sourceResource: string;
        workContributor: string;
        workMedium: string;
        orcidLink: string;
      };
      groupTypes: {
        vocab: string;
        citation: string;
        book: string;
      };
      overview: {
        kicker: string;
        subtitle: string;
        stats: {
          totalInfos: string;
          connections: string;
          words: string;
          sentences: string;
          languages: string;
          collections: string;
          revisions: string;
          storyboards: string;
          texts: string;
        };
        statsCardTitle: string;
        statsCardHint: string;
        knownChartTitle: string;
        checksChartTitle: string;
        known: string;
        unknown: string;
        excluded: string;
        availableChecks: string;
        unavailableChecks: string;
        quickActionsTitle: string;
        quickActionsText: string;
        browseList: string;
        viewCollections: string;
        addInfo: string;
        seedMock: string;
        seedSuccess: string;
        seedFail: string;
        focusTitle: string;
        focusBody1: string;
        focusBody2: string;
        transferTitle: string;
        transferBody: string;
        export: string;
        import: string;
        exporting: string;
        importing: string;
        exportProgress: string;
        importProgress: string;
        progressUnknown: string;
        exportReady: string;
        exportFail: string;
        importDone: string;
        importStageInfos: string;
        importStageConnections: string;
        importStageCollections: string;
        importLiveProgress: string;
        importSummary: string;
        importFail: string;
      };
      modules: {
        storyboardsTitle: string;
        storyboardsSubtitle: string;
        storyboardsNewLabel: string;
        storyboardsNewPlaceholder: string;
        storyboardsCreate: string;
        storyboardsExisting: string;
        storyboardsEmpty: string;
        storyboardsEditor: {
          titleRequired: string;
          idMissing: string;
          createdStatus: string;
          savedStatus: string;
          invalidLinkStatus: string;
          selfLinkStatus: string;
          startIncomingStatus: string;
          endOutgoingStatus: string;
          creatingAction: string;
          savingAction: string;
          saveAction: string;
          runAction: string;
          shareWorking: string;
          shareCreateAction: string;
          shareCopyAction: string;
          shareRevokeAction: string;
          descriptionLabel: string;
          descriptionPlaceholder: string;
          rootGraphAction: string;
          addStaticAction: string;
          addCardAction: string;
          addGroupAction: string;
          deleteNodeAction: string;
          deleteLinkAction: string;
          nodeTitleLabel: string;
          nodeDescriptionLabel: string;
          staticTypeLabel: string;
          staticBodyLabel: string;
          mediaUrlLabel: string;
          selectedNotionLabel: string;
          selectNotionAction: string;
          checkcardsLoading: string;
          checkcardsLoadFailed: string;
          checkcardsEmpty: string;
          selectCheckingcardHint: string;
          overlaySearchLabel: string;
          overlaySearchPlaceholder: string;
          overlaySearching: string;
          overlayNoLinkedCheckcards: string;
          overlayResultCardsSuffix: string;
          notionIdLabel: string;
          directionLabel: string;
          enterGroupAction: string;
          structuralNodeHelp: string;
          linkLabelLabel: string;
          linkLabelPlaceholder: string;
          displayModeLabel: string;
          displayModeNewScreen: string;
          displayModeExpand: string;
          displayModeFixedHelp: string;
          inspectorEmpty: string;
          overviewNoDescription: string;
          overviewNodes: string;
          overviewLinks: string;
          overviewStatic: string;
          overviewCards: string;
          overviewGroups: string;
          overviewStaticBreakdown: string;
          sharedLinkLabel: string;
          sharedLinkEmpty: string;
          shareUpdateError: string;
          shareCopied: string;
          shareCopyError: string;
          staticTypeText: string;
          staticTypeVideo: string;
          staticTypeAudio: string;
          staticTypeImage: string;
          staticTypeOther: string;
          cardDirectionFrontToBack: string;
          cardDirectionBackToFront: string;
          nodeKindStatic: string;
          nodeKindCard: string;
          nodeKindGroup: string;
          nodeKindStart: string;
          nodeKindEnd: string;
          edgeKindDependency: string;
          edgeKindRight: string;
          edgeKindWrong: string;
        };
        storyboardsRuntime: {
          statusMissingParams: string;
          statusNotFound: string;
          statusLoadFailed: string;
          statusLoadSharedFailed: string;
          choiceFallback: string;
          backAction: string;
          kicker: string;
          titleFallback: string;
          restartAction: string;
          loading: string;
          blockUntitled: string;
          openMediaAction: string;
          notionLabel: string;
          notionNotSet: string;
          directionLabel: string;
          directionFrontToBack: string;
          directionBackToFront: string;
          cardCheckTypeLabel: string;
          cardLoading: string;
          cardPromptLabel: string;
          cardAnswerLabel: string;
          cardAnswerPlaceholder: string;
          cardSubmitAction: string;
          cardRevealLabel: string;
          cardContinueAction: string;
          cardAutoAdvance: string;
          noCardOutcomeLink: string;
          groupCompleted: string;
          finished: string;
          rightAction: string;
          wrongAction: string;
          chapterChooseLabel: string;
          chapterFinishAction: string;
          chapterRestartFromStartAction: string;
        };
        textsTitle: string;
        textsSubtitle: string;
        textsNewLabel: string;
        textsNewPlaceholder: string;
        textsCreate: string;
        textsExisting: string;
        textsEmpty: string;
        loading: string;
        loadFailed: string;
        createFailed: string;
        draftInfo: string;
      };
      statistics: {
        loading: string;
        unavailable: string;
        checkedAnswers: string;
        avgCorrectness: string;
        totalPoints: string;
        participants: string;
        avgResponseTime: string;
        previous: string;
        next: string;
        noStatsForScope: string;
        loadFailed: string;
        participantsTable: string;
        participantsTableHint: string;
        total: string;
        wrong: string;
        correct: string;
        participant: string;
        score: string;
        correctness: string;
        totalTime: string;
        avgTime: string;
        answers: string;
        ascending: string;
        descending: string;
        notAvailable: string;
        answersXAxisHint: string;
        scoreHint: string;
        averageResponseTimeLine: string;
        hideParticipant: string;
        showParticipant: string;
        openSlide: string;
        modules: {
          scoreLineTitle: string;
          scoreLineSubtitle: string;
          knownessWordsTitle: string;
          knownessWordsSubtitle: string;
          knownessLineTitle: string;
          knownessLineSubtitle: string;
          roundScoreGainTitle: string;
          roundScoreGainSubtitle: string;
          responseTimeComparisonTitle: string;
          responseTimeComparisonSubtitle: string;
          responseTimePyramidTitle: string;
          responseTimePyramidSubtitle: string;
          responseTimeSplitTitle: string;
          responseTimeSplitSubtitle: string;
          barChartTitle: string;
          barChartSubtitle: string;
          pyramidRankingTitle: string;
          pyramidRankingSubtitle: string;
          numberedMetricsTitle: string;
          numberedMetricsSubtitle: string;
        };
      };
      list: {
        kicker: string;
        subtitle: string;
        searchTitle: string;
        searchPlaceholder: string;
        cardCount: string;
        cardTypeInfo: string;
        cardTypeConnection: string;
        cardTypeVocab: string;
        modes: {
          detail: string;
          collection: string;
          list: string;
        };
        selectedTitle: string;
        selectedEmpty: string;
        selectedHint: string;
        noMatch: string;
        addInfo: string;
        loadMore: string;
        loading: string;
        ready: string;
        progressUnlimited: string;
        computedSampleTitle: string;
        computedPromptLabel: string;
        computedAnswerLabel: string;
        computedLoading: string;
        computedEmpty: string;
        editInfo: string;
        deleteConnection: string;
        deleteConnectionConfirm: string;
        deleteConnectionFailed: string;
        searchOnlyHint: string;
        typeLabel: string;
        sortLabel: string;
        sortRelevance: string;
        sortLabelAsc: string;
        sortLabelDesc: string;
        sortTypeAsc: string;
        sortTypeDesc: string;
        selectionCount: string;
        selectAllVisible: string;
        clearSelection: string;
        openSelected: string;
        openSelectedDisabled: string;
        constraintLabel: string;
        constraintPlaceholder: string;
        viewLabel: string;
        viewDetails: string;
        viewWide: string;
        viewGrid: string;
        selectedStackTitle: string;
        selectedStackHint: string;
        openInCollection: string;
        openInDependencies: string;
        removeFromStack: string;
        detailColumnName: string;
        detailColumnType: string;
        detailColumnId: string;
        showFilters: string;
        hideFilters: string;
        filtersOptionalHint: string;
        filtersLoading: string;
        typeFilterLanguage: string;
        typeFilterLanguageA: string;
        typeFilterLanguageB: string;
        typeFilterOriginalLanguage: string;
        typeFilterDoi: string;
        typeFilterSourceKind: string;
        typeFilterLocator: string;
        typeFilterCitationText: string;
      };
      filters: {
        title: string;
        clear: string;
        languageA: string;
        languageB: string;
        topic: string;
        level: string;
        placeholderLanguageA: string;
        placeholderLanguageB: string;
        placeholderTopic: string;
        placeholderLevel: string;
      };
      add: {
        kicker: string;
        subtitle: string;
        tabs: {
          info: string;
          connection: string;
          group: string;
        };
        tabDesc: {
          info: string;
          connection: string;
          group: string;
        };
        info: {
          typeLabel: string;
          labelLabel: string;
          labelPlaceholder: string;
          languageLabel: string;
          languagePlaceholder: string;
          notesLabel: string;
          notesPlaceholder: string;
          sourceKindLabel: string;
          sourceKindPlaceholder: string;
          sourceResourceTypeLabel: string;
          sourceResourceLabel: string;
          sourceResourcePlaceholder: string;
          sourceBibleBookLabel: string;
          sourceBibleBookPlaceholder: string;
          sourceBibleRestLabel: string;
          sourceBibleRestPlaceholder: string;
          quoteTextLabel: string;
          quoteTextPlaceholder: string;
          workLanguageLabel: string;
          workLanguagePlaceholder: string;
          workOriginalLanguageLabel: string;
          workOriginalLanguagePlaceholder: string;
          workDoiLabel: string;
          workDoiPlaceholder: string;
          mediaTypeLabel: string;
          mediaTypePlaceholder: string;
          mediaPublisherLabel: string;
          mediaPublisherPlaceholder: string;
          mediaPublicationPlaceLabel: string;
          mediaPublicationPlacePlaceholder: string;
          mediaPublicationYearLabel: string;
          mediaPublicationYearPlaceholder: string;
          mediaPagesLabel: string;
          mediaPagesPlaceholder: string;
          mediaIsbnLabel: string;
          mediaIsbnPlaceholder: string;
          mediaCoverLabel: string;
          mediaCoverPlaceholder: string;
          mediaHeightLabel: string;
          mediaHeightPlaceholder: string;
          mediaLengthLabel: string;
          mediaLengthPlaceholder: string;
          mediaWidthLabel: string;
          mediaWidthPlaceholder: string;
          mediaWeightLabel: string;
          mediaWeightPlaceholder: string;
          mediaCollectionLabel: string;
          mediaCollectionPlaceholder: string;
          mediaLocationLabel: string;
          mediaLocationPlaceholder: string;
          sourceUrlLabel: string;
          sourceUrlPlaceholder: string;
          sourceAccessedDateLabel: string;
          sourceAccessedDatePlaceholder: string;
          referenceTitle: string;
          referenceToggle: string;
          referenceMissingSource: string;
          computedLabel: string;
          computedPlaceholder: string;
          computedAnswerLabel: string;
          computedAnswerPlaceholder: string;
          computedAnswerRequired: string;
          computedAnswerMissingOutput: string;
          computedRequired: string;
          computedInvalid: string;
          computedInvalidName: string;
          computedDuplicateName: string;
          computedPreview: string;
          computedPreviewTitle: string;
          computedPreviewFail: string;
        save: string;
        update: string;
        saved: string;
        updated: string;
        failed: string;
      };
        connection: {
          typeLabel: string;
          languageLabel: string;
          languagePlaceholder: string;
          wordLabel: string;
          wordPlaceholder: string;
          wordALabel: string;
          wordAPlaceholder: string;
          wordBLabel: string;
          wordBPlaceholder: string;
          sentenceLabel: string;
          sentencePlaceholder: string;
          topicLabel: string;
          topicPlaceholder: string;
          quoteLabel: string;
          quotePlaceholder: string;
          sourceLabel: string;
          sourcePlaceholder: string;
          referencedInfoTypeLabel: string;
          referencedInfoLabel: string;
          referencedInfoPlaceholder: string;
          workLabel: string;
          workPlaceholder: string;
          contributorTypeLabel: string;
          contributorLabel: string;
          contributorPlaceholder: string;
          contributorRoleLabel: string;
          contributorRolePlaceholder: string;
          mediumLabel: string;
          mediumPlaceholder: string;
          orcidEntityHeader: string;
          orcidHeader: string;
          sourceResourceTypeLabel: string;
          sourceResourceLabel: string;
          sourceResourcePlaceholder: string;
          noteLabel: string;
          notePlaceholder: string;
          save: string;
          saved: string;
          failed: string;
          pairExists: string;
          selectWordLanguage: string;
          selectWordTopic: string;
          selectTwoWords: string;
          selectLanguageSentence: string;
          selectReference: string;
          selectSourceResource: string;
          selectQuoteLanguage: string;
          selectWorkContributor: string;
          selectWorkMedium: string;
          selectOrcidLink: string;
        };
        group: {
          typeLabel: string;
          languageALabel: string;
          languageAPlaceholder: string;
          wordATagsLabel: string;
          wordATagsPlaceholder: string;
          wordBTagsLabel: string;
          wordBTagsPlaceholder: string;
          translationTagsLabel: string;
          translationTagsPlaceholder: string;
          wordALabel: string;
          wordAPlaceholder: string;
          languageBLabel: string;
          languageBPlaceholder: string;
          wordBLabel: string;
          wordBPlaceholder: string;
          citationTitleLabel: string;
          citationTitlePlaceholder: string;
          citationQuoteTextLabel: string;
          citationQuoteTextPlaceholder: string;
          citationLanguageLabel: string;
          citationLanguagePlaceholder: string;
          citationSourceKindLabel: string;
          citationBibleBookLabel: string;
          citationBibleBookPlaceholder: string;
          citationBibleRestLabel: string;
          citationBibleRestPlaceholder: string;
          citationChurchDocumentLabel: string;
          citationChurchDocumentPlaceholder: string;
          citationWorkLabel: string;
          citationWorkPlaceholder: string;
          citationBookLabel: string;
          citationBookPlaceholder: string;
          citationLocatorValueLabel: string;
          citationLocatorValuePlaceholder: string;
          bookTitleLabel: string;
          bookTitlePlaceholder: string;
          workTitleLabel: string;
          workTitlePlaceholder: string;
          contributorsLabel: string;
          addContributor: string;
          removeContributor: string;
          bookMissingTitle: string;
          bookMissingWork: string;
          bookMissingContributor: string;
          citationMissingQuote: string;
          citationMissingSource: string;
          saveVocab: string;
          saveCitation: string;
          saveBook: string;
          savedVocab: string;
          savedCitation: string;
          savedBook: string;
          failed: string;
          pairExistsA: string;
          pairExistsB: string;
          selectBoth: string;
        };
      };
      collections: {
        listKicker: string;
        listSubtitle: string;
        searchPlaceholder: string;
        emptyTitle: string;
        emptyAction: string;
        countLabel: string;
        collectionLabel: string;
        noNotes: string;
        defaultName: string;
        itemCountLabel: string;
        detailKicker: string;
        detailSubtitle: string;
        noCards: string;
        detailFocusTitle: string;
        detailFocusBody: string;
        deleteConfirm: string;
        deleteFail: string;
        createKicker: string;
        createSubtitle: string;
        collectionInfoTitle: string;
        nameLabel: string;
        namePlaceholder: string;
        notesLabel: string;
        notesPlaceholder: string;
        saveRequiredName: string;
        saveSuccess: string;
        saveFail: string;
        findCardsTitle: string;
        searchCardsPlaceholder: string;
        addToCollection: string;
        added: string;
        selectedCardsTitle: string;
        selectedCountLabel: string;
        noSelected: string;
        remove: string;
        loading: string;
        ready: string;
      };
      revision: {
        settingsKicker: string;
        settingsSubtitle: string;
        identitySettingsTitle: string;
        showSectionAction: string;
        hideSectionAction: string;
        globalThresholdsTitle: string;
        globalQuestionCorrectnessLabel: string;
        globalDependencyCorrectnessLabel: string;
        runKicker: string;
        shareRunKicker: string;
        modeSummary: string;
        modeLabel: string;
        modeValue: string;
        modeValueRandomOnce: string;
        modeValueLevels: string;
        modeValueTemporal: string;
        modeDescriptionRandom: string;
        modeDescriptionRandomOnce: string;
        modeDescriptionLevels: string;
        modeDescriptionTemporal: string;
        commonSettingsTitle: string;
        specificSettingsTitle: string;
        settingsImpactTitle: string;
        settingsImpactIntro: string;
        dependencyThresholdDisabledHint: string;
        effectCollection: string;
        effectTries: string;
        effectMinCorrectness: string;
        effectConsiderDependenciesOn: string;
        effectConsiderDependenciesOff: string;
        effectDependencyThreshold: string;
        effectStackSize: string;
        effectLevels: string;
        effectCardsPerSession: string;
        effectReviewer: string;
        levelsLabel: string;
        stackLabel: string;
        levelsCurrentLabel: string;
        levelsCountsLabel: string;
        levelsStackLabel: string;
        triesLabel: string;
        dependencyThresholdLabel: string;
        minCorrectnessLabel: string;
        compareLabel: string;
        compareBidirectional: string;
        comparePrefix: string;
        compareAnchors: string;
        considerDependenciesLabel: string;
        considerDependenciesOn: string;
        considerDependenciesOff: string;
        temporalUnknownLabel: string;
        temporalKnownLabel: string;
        checkLabel: string;
        checkValue: string;
        cardsPerSessionLabel: string;
        reviewerLabel: string;
        shareKicker: string;
        shareTitle: string;
        shareBody: string;
        shareAction: string;
        shareWorking: string;
        shareLinkLabel: string;
        shareCopyAction: string;
        shareCopied: string;
        shareCopyError: string;
        shareError: string;
        shareListTitle: string;
        shareListLoading: string;
        shareListEmpty: string;
        shareRevokeAction: string;
        shareRevoked: string;
        shareInvalid: string;
        shareLoading: string;
        previewTitle: string;
        previewBody1: string;
        previewBody2: string;
        start: string;
        progressTitle: string;
        loading: string;
        loadingComputed: string;
        error: string;
        empty: string;
        completed: string;
        vocabLabel: string;
        infoLabel: string;
        answerLabel: string;
        answerPlaceholder: string;
        answerPlaceholderComputed: string;
        quoteMissingPlaceholder: string;
        quoteProgress: string;
        answerSentenceLabel: string;
        answerSentencePlaceholder: string;
        checkAnswer: string;
        skip: string;
        nextQuestion: string;
        hintSelectLanguage: string;
        hintReview: string;
        markDone: string;
        correct: string;
        tryAgain: string;
        noLanguages: string;
        matchLabel: string;
        knownessTitle: string;
        knownessEmpty: string;
        knownessStats: string;
        knownessLast: string;
        knownessHint: string;
        dependenciesBlocked: string;
        revealModeLabel: string;
        revealModeAfterIncorrect: string;
        showAnswer: string;
        correctAnswerLabel: string;
        live: {
          hostKicker: string;
          hostTitle: string;
          joinKicker: string;
          joinTitle: string;
          joinCodeLabel: string;
          joinUrlLabel: string;
          qrLabel: string;
          statusLabel: string;
          sessionModeLabel: string;
          modeSimultaneous: string;
          modeAsynchronous: string;
          hostViewModeLabel: string;
          hostViewPanel: string;
          hostViewQuestion: string;
          hostViewScore: string;
          participantViewModeLabel: string;
          participantViewFollowHost: string;
          participantViewQuestion: string;
          participantViewScore: string;
          participantViewFullscreen: string;
          viewModeLabel: string;
          viewModeWindow: string;
          viewModeFullscreen: string;
          sessionSettingsLabel: string;
          statusLobby: string;
          statusRunning: string;
          statusRevealed: string;
          statusFinished: string;
          roundsLabel: string;
          participantsTitle: string;
          scoreboardTitle: string;
          pointsTitle: string;
          finalScoreTitle: string;
          podiumTitle: string;
          currentRoundTitle: string;
          questionTitle: string;
          waitingForHostQuestion: string;
          waitingForPublishedRound: string;
          sessionNotStarted: string;
          hiddenBeforeStart: string;
          noParticipants: string;
          participantNameLabel: string;
          classNameLabel: string;
          classGroupsLabel: string;
          classGroupsPlaceholder: string;
          classGroupsHelp: string;
          joinAction: string;
          joiningAction: string;
          joinedWaiting: string;
          connectionError: string;
          scoreHistoryChartAria: string;
          submitAnswer: string;
          submitted: string;
          publishCurrentRound: string;
          checkAndReveal: string;
          nextQuestionAction: string;
          stopNextQuestionTimerAction: string;
          loading: string;
          unsupportedPromptType: string;
          addPathAction: string;
          fragmentLabel: string;
          correctFragmentLabel: string;
          participantAnswerPlaceholder: string;
          selectMatchingPairPrompt: string;
          wordLanguagePromptPrefix: string;
          loadRoundsError: string;
          createSessionError: string;
          answerStatusAnswered: string;
          answerStatusWaiting: string;
          answerStatusCorrect: string;
          answerStatusIncorrect: string;
          hostUrlLabel: string;
          presenterUrlLabel: string;
          detailLaunchTitle: string;
          openLoginWallAction: string;
          openHostWallAction: string;
          openScreenWallAction: string;
          reloginQrAction: string;
          reloginQrHelp: string;
          participantPointsLabel: string;
          groupNameLabel: string;
          groupNamePlaceholder: string;
          groupsListTitle: string;
          groupsListEmpty: string;
          loadGroupAction: string;
          currentGroupLabel: string;
          saveGroupAction: string;
          groupSavedMessage: string;
          groupSaveError: string;
          startNextGroupError: string;
          resumeSessionHint: string;
          newSessionAction: string;
          resetSessionAction: string;
          refreshLinksAction: string;
          closeSessionAction: string;
          startTimerAction: string;
          timerLabel: string;
          timeExpired: string;
          addParticipantAction: string;
          addParticipantPlaceholder: string;
          removeParticipantAction: string;
          removeParticipantConfirmLabel: string;
          cardsLeftLabel: string;
          preparingCardsLabel: string;
          preparedCardsLabel: string;
          reloginRequestsTitle: string;
          allowReloginAction: string;
          reloginPendingMessage: string;
          symbolsLegend: string;
          roundGainTitle: string;
          roundGainDetailsTitle: string;
          reviewQuestionsTitle: string;
          previousQuestionAction: string;
          pauseTimerAction: string;
          resumeTimerAction: string;
          switchParticipantAction: string;
          useExistingParticipantAction: string;
          chooseDifferentNameAction: string;
          duplicateNameInfoLabel: string;
          roundReasonBase: string;
          roundReasonFirst: string;
          roundReasonSpeed: string;
          roundReasonStreak: string;
          roundReasonWrong: string;
          roundReasonFirstWrong: string;
          roundReasonWrongStreak: string;
          asyncScoreContextHelp: string;
          bonusExplanation: string;
          showScoreOverlayAction: string;
          hideScoreOverlayAction: string;
          factorBaseLabel: string;
          factorFirstLabel: string;
          factorSpeedLabel: string;
          factorStreakLabel: string;
          factorWrongLabel: string;
          factorFirstWrongLabel: string;
          factorWrongStreakLabel: string;
          streakLabel: string;
          onFirstAnswerLabel: string;
          onAllAnsweredLabel: string;
          roundTimerLabel: string;
          roundTimerEnabledLabel: string;
          roundTimerSecondsLabel: string;
          roundTimerExpireLabel: string;
          actionTimerLabel: string;
          timerEnabledLabel: string;
          timerSecondsLabel: string;
          onTimerExpiredLabel: string;
          nextQuestionBehaviorLabel: string;
          nextQuestionModeLabel: string;
          nextQuestionModeManual: string;
          nextQuestionModeTimer: string;
          nextQuestionTimerSecondsLabel: string;
          nextQuestionTimerLabel: string;
          bonusTimerLabel: string;
          bonusTimerEnabledLabel: string;
          bonusTimerSecondsLabel: string;
          bonusTimerStartLabel: string;
          bonusTimerStartAfterFirst: string;
          bonusTimerStartRound: string;
          basePointsLabel: string;
          firstCorrectBonusLabel: string;
          firstBonusModeLabel: string;
          firstBonusModeFirstAnswer: string;
          firstBonusModeFirstCorrect: string;
          wrongAnswerPenaltyLabel: string;
          firstWrongPenaltyLabel: string;
          firstWrongPenaltyModeLabel: string;
          firstWrongPenaltyModeOverallFirst: string;
          firstWrongPenaltyModeFirstWrong: string;
          speedBonusEnabledLabel: string;
          speedBonusMaxLabel: string;
          speedGrowthLabel: string;
          streakBaseBonusLabel: string;
          streakBonusEnabledLabel: string;
          streakGrowthLabel: string;
          streakLimitLabel: string;
          wrongStreakPenaltyEnabledLabel: string;
          wrongStreakBasePenaltyLabel: string;
          wrongStreakGrowthLabel: string;
          wrongStreakLimitLabel: string;
          presetLabel: string;
          presetBalancedDuel: string;
          presetFirstStrike: string;
          presetSprintRace: string;
          presetAccuracyFocus: string;
          presetStreakMaster: string;
          presetMarathonClassic: string;
          presetAsyncChallenge: string;
          presetCustom: string;
          summaryTitle: string;
          summaryPreset: string;
          summaryPoints: string;
          summaryTimerEnabled: string;
          summaryTimerDisabled: string;
          summaryScoringFlow: string;
          summaryBasePoints: string;
          summarySpeedCurve: string;
          summaryAsyncPreset: string;
          summarySyncPreset: string;
          summaryBonusNone: string;
          summaryBonusFirst: string;
          summaryBonusSpeed: string;
          summaryBonusStreak: string;
          summaryWrongPenalty: string;
          summaryFirstWrongPenalty: string;
          summaryNoWrongPenalty: string;
          summaryBonusSpeedDisabled: string;
          summaryBonusStreakDisabled: string;
          summaryEvaluationFlow: string;
          summaryPointsTotalHint: string;
          summaryStreak: string;
          summaryActions: string;
          summaryTypeLineSync: string;
          summaryTypeLineAsync: string;
          summaryBasePointsDetailed: string;
          summaryBonusItem: string;
          summaryBonusesList: string;
          summaryRangeLine: string;
          summaryBonusesActive: string;
          summaryBonusesNone: string;
          summaryBonusTimerDetail: string;
          summaryBonusTimerDisabled: string;
          summaryStreakDetail: string;
          summaryStreakDisabled: string;
          summaryAllAnsweredDetail: string;
          summaryRoundTimerDetail: string;
          summaryRoundTimerDisabled: string;
          summaryNextQuestionManual: string;
          summaryNextQuestionTimer: string;
          summaryActionFlowDetail: string;
          summaryActionTimerOnlyDetail: string;
          summaryActionTimerDisabledDetail: string;
          bonusUsageLabel: string;
          bonusUsageAll: string;
          bonusUsageNone: string;
          bonusUsageFirst: string;
          bonusUsageSpeed: string;
          bonusUsageStreak: string;
          optionDoNothing: string;
          optionStartTimer: string;
          optionRevealScore: string;
          optionRevealNext: string;
          optionYes: string;
          optionNo: string;
          optionLinear: string;
          optionExponential: string;
          optionLimited: string;
          waitingForRevealLabel: string;
          selectedPathsLabel: string;
          removePathAction: string;
          columnPrefixLabel: string;
          trueLabel: string;
          falseLabel: string;
          scoreUnit: string;
          activeSessionsTitle: string;
          roleLabel: string;
          roleHost: string;
          roleParticipant: string;
          roleBoth: string;
          hostSessionsTitle: string;
          participantSessionsTitle: string;
          sharedRoleSessionsTitle: string;
          noHostSessions: string;
          noParticipantSessions: string;
          noSessionsForRole: string;
          yourScoreLabel: string;
          connectedLabel: string;
          disconnectedLabel: string;
          refreshAction: string;
          closedStatus: string;
        };
      };
      lookup: {
        searchFailed: string;
        createFailed: string;
        createNew: string;
        saving: string;
        loadMore: string;
      };
      graph: {
        kicker: string;
        subtitle: string;
        palette: string;
        inspector: string;
        typeLabel: string;
        save: string;
        saveSuccess: string;
        saveFail: string;
        preview: string;
        previewLabel: string;
        previewBreakdown: string;
        emptyInspector: string;
        nameLabel: string;
        namePlaceholder: string;
        outputLabel: string;
        outputPlaceholder: string;
        duplicateName: string;
        invalidName: string;
        nameHint: string;
        regenerateRandom: string;
        deleteNode: string;
        noParams: string;
        nodeTypes: {
          inputRandom: string;
          inputConst: string;
          inputList: string;
          add: string;
          sub: string;
          mul: string;
          div: string;
          pow: string;
          exp: string;
          log: string;
          abs: string;
          min: string;
          max: string;
          floor: string;
          ceil: string;
          round: string;
          mod: string;
          concat: string;
          trim: string;
          output: string;
        };
        handleLabels: {
          input: string;
          add: string;
          sub: string;
          numerator: string;
          denominator: string;
          base: string;
          exponent: string;
          value: string;
          index: string;
          name: string;
          text: string;
          start: string;
          end: string;
        };
        listLabel: string;
        listPlaceholder: string;
        tagLabel: string;
        tagPlaceholder: string;
        languageLabel: string;
        languagePlaceholder: string;
        scopeLabel: string;
        scopeAny: string;
        scopeTranslation: string;
        scopeWordA: string;
        scopeWordB: string;
        infoTypeLabel: string;
        specificInfoLabel: string;
        specificInfoPlaceholder: string;
        connectionTypeLabel: string;
        connectionIdLabel: string;
        connectionIdPlaceholder: string;
      };
    };
  };
  faq: {
    title: string;
    items: Array<{ q: string; a: string }>;
  };
  legal: {
    title: string;
    items: Array<{ title: string; desc: string }>;
  };
  account: {
    title: string;
    subtitle: string;
    overviewTitle: string;
    overviewLead: string;
    sections: {
      overview: string;
      profile: string;
      roles: string;
      security: string;
    };
    menuLabel: string;
    loginIdLabel: string;
    loginIdHint: string;
    nicknameLabel: string;
    nicknameHint: string;
    nicknamePlaceholder: string;
    nicknameToggle: string;
    nicknameAction: string;
    nicknameWorking: string;
    nicknameSuccess: string;
    nicknameError: string;
    passwordTitle: string;
    currentPassword: string;
    newPassword: string;
    confirmPassword: string;
    passwordToggle: string;
    passwordAction: string;
    passwordWorking: string;
    passwordSuccess: string;
    passwordError: string;
    secureModeTitle: string;
    secureModeNote: string;
    secureModeEnable: string;
    secureModeDisable: string;
    logoutAction: string;
    roles: {
      lead: string;
      searchPlaceholder: string;
      reachableToggle: string;
      fullscreenEnter: string;
      fullscreenExit: string;
      noFilters: string;
      loading: string;
      noNodes: string;
      panelTitle: string;
      panelEmpty: string;
      legendTitle: string;
      edgeTitle: string;
      edgeDeleteAction: string;
      edgeDeleteWorking: string;
      edgeDeleteSuccess: string;
      edgeDeleteError: string;
      linkDraftTitle: string;
      linkDraftHint: string;
      linkWorking: string;
      linkSuccess: string;
      linkError: string;
      createOwnedRole: string;
      createOwnedRoleHint: string;
      createRoleTitle: string;
      createRoleNickLabel: string;
      createRoleKindLabel: string;
      createRoleRelationLabel: string;
      createRoleAction: string;
      createRoleWorking: string;
      createRoleSuccess: string;
      createRoleError: string;
      dataAddTitle: string;
      dataFieldLabel: string;
      dataKindLabel: string;
      dataKindData: string;
      dataKindKey: string;
      dataValueLabel: string;
      dataAddAction: string;
      dataAddWorking: string;
      dataAddSuccess: string;
      dataAddError: string;
      dataEditTitle: string;
      dataEditAction: string;
      dataEditWorking: string;
      dataEditSuccess: string;
      dataEditError: string;
      dataDeleteAction: string;
      dataDeleteWorking: string;
      dataDeleteSuccess: string;
      dataDeleteError: string;
      dataOwnerLabel: string;
      dataShareTitle: string;
      dataShareTargetLabel: string;
      dataSharePermissionLabel: string;
      dataShareAction: string;
      dataShareWorking: string;
      dataShareSuccess: string;
      dataShareError: string;
      shareRoleTitle: string;
      shareRoleTargetLabel: string;
      shareRoleRelationLabel: string;
      shareRoleAction: string;
      shareRoleWorking: string;
      shareRoleSuccess: string;
      shareRoleError: string;
      shareRoleHint: string;
      recoveryPrepareAction: string;
      recoveryPrepareWorking: string;
      recoveryPrepareSuccess: string;
      recoveryPrepareError: string;
      recoveryPlanTitle: string;
      recoveryPlanHint: string;
      recoveryPlanNeedsShares: string;
      recoveryPlanLabel: string;
      recoveryPlanError: string;
      recoveryShareWorking: string;
      recoveryShareSuccess: string;
      recoveryShareError: string;
      recoveryActivateAction: string;
      recoveryActivateWorking: string;
      recoveryActivateSuccess: string;
      recoveryActivateError: string;
      contextAddRoleTitle: string;
      contextAddRolePlaceholder: string;
      contextAddRoleAction: string;
      contextAddRoleWorking: string;
      contextAddRoleSuccess: string;
      contextAddRoleError: string;
      linkPermissionNeeded: string;
      contextAddData: string;
      contextPrepareRecovery: string;
      contextClose: string;
      pendingTitle: string;
      pendingAction: string;
      pendingWorking: string;
      pendingError: string;
      pendingEmpty: string;
      pendingAccept: string;
      pendingDataTitle: string;
      pendingDataAction: string;
      pendingDataWorking: string;
      pendingDataError: string;
      pendingDataEmpty: string;
      parentsTitle: string;
      parentsAction: string;
      parentsWorking: string;
      parentsError: string;
      parentsRemoved: string;
      parentsRemoveAction: string;
      verifyTitle: string;
      verifyAction: string;
      verifyWorking: string;
      verifyError: string;
      verifyHashOk: string;
      verifyHashIssue: string;
      verifySigOk: string;
      verifySigIssue: string;
      verifyRoleSigned: string;
      relationNotesTitle: string;
      relationOwner: string;
      relationWrite: string;
      relationRead: string;
      viewGraph: string;
      viewDetails: string;
      cancelAction: string;
      incomingTitle: string;
      outgoingTitle: string;
      none: string;
    };
  };
  footer: {
    headline: string;
    contact: string;
    imprint: string;
    security: string;
  };
  loginCard: {
    title: string;
    contextDefault: string;
  };
  cg: {
    title: string;
    subtitle: string;
    loginCta: string;
    landing: {
      logo: string;
      headline: string;
      headlineAccent: string;
      sub: string;
      features: Array<{ icon: string; title: string; desc: string }>;
      cta: string;
      linkCogita: string;
      linkHome: string;
    };
    home: {
      title: string;
      sub: string;
      newLibrary: string;
      wizardTitle: string;
      nameLabel: string;
      namePlaceholder: string;
      templateLabel: string;
      createAction: string;
      creating: string;
      cancel: string;
      loadFailed: string;
      createFailed: string;
      deleteFailed: string;
      deleteConfirm: string;
      deleteCancel: string;
      emptyTitle: string;
      emptyDesc: string;
      templates: {
        vocabulary: { name: string; desc: string };
        phonebook:  { name: string; desc: string };
        lesson:     { name: string; desc: string };
        custom:     { name: string; desc: string };
      };
      templateLabels: Record<string, string>;
      backToCogita: string;
    };
    library: {
      kicker: Record<string, string>;
      renameFailed: string;
      saveAction: string;
      cancelAction: string;
      nodeTypes: string;
      fields: string;
      nodes: string;
      browseNodes: string;
      editSchema: string;
      schemaPreview: string;
      noFields: string;
      subentity: string;
      goTo: string;
      allNodes: string;
      schemaEditor: string;
      allLibraries: string;
    };
    studio: {
      title: string;
      sub: string;
      addField: string;
      fieldName: string;
      fieldType: string;
      multiValue: string;
      range: string;
      addAction: string;
      addTypeTitle: string;
      typeName: string;
      typePlaceholder: string;
      isSubentity: string;
      addTypeAction: string;
      addingType: string;
      noFields: string;
      deleteType: string;
      addTypeFailed: string;
      addFieldFailed: string;
      deleteFieldFailed: string;
      deleteTypeFailed: string;
      labelIndex: string;
      addFormPreview: string;
    };
    graph: {
      title: string;
      searchPlaceholder: string;
      allTypes: string;
      addNode: string;
      labelPlaceholder: string;
      addAction: string;
      adding: string;
      loadFailed: string;
      createFailed: string;
      deleteFailed: string;
      labelColumn: string;
      typeColumn: string;
      noNodes: string;
      noNodesDesc: string;
    };
    node: {
      backAction: string;
      labelSection: string;
      fieldsSection: string;
      connectionsSection: string;
      noLabel: string;
      saveAction: string;
      cancelAction: string;
      saveFailed: string;
      fieldSaveFailed: string;
      deleteValueFailed: string;
      multi: string;
      outgoing: string;
      incoming: string;
      created: string;
      updated: string;
    };
    nav: {
      allLibraries: string;
      overview: string;
      schemaEditor: string;
      nodes: string;
      signOut: string;
      switchToCogita: string;
    };
  };
};
