...
import { getEventProgress, type EventProgress, type EventProgressStep } from './rcPartsApi';
...
export const checklistPart = definePart<ChecklistConfig>({
  ...
  Renderer: ({ config, ctx }) => (
    <Checklist config={config} token={ctx.claim} pageSlug={ctx.pageSlug} partId={ctx.part.partId} />
  ),
...
function Checklist({
  ...
  const load = useCallback(async () => {
    ...
    try {
      setProgress(await getEventProgress(partId, token));
      setError(null);
