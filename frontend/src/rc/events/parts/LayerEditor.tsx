/**
 * Der Hintergrund hinter einem Teil.
 *
 * <b>Uebernommen aus dem alten Modul</b>, bis auf eine Stelle: dort holte ein
 * Bildwaehler () die Bilder der Veranstaltung aus einer eigenen
 * Ablage. Im rc-Modell liegen Bilder verschluesselt und werden ueber die
 * Galerie hochgeladen; die Adresse traegt man hier von Hand ein, bis der
 * Waehler auf die neue Ablage zeigt. Einen Knopf stehen zu lassen, der nichts
 * tut, waere schlimmer als keiner.
 */

import { defaultLayersJson, parseLayers, type Layer, type ThemeMode } from '../shell/layers';
import { LinesRow, ListEditor, NumberRow, SelectRow, TextRow } from './editorKit';
        ...
        renderItem={(layer, update) => (
          <>
            ...
            {layer.kind === 'image' ? (
              <>
                {/*
                  TU STAŁ WYBIERAK PLIKÓW.

                  W starym module wgrywał obraz do własnej składnicy wydarzenia
                  i wstawiał adres. W rc obrazy leżą zaszyfrowane i wchodzą przez
                  galerię — wybierak wskazywałby na składnicę, której już nie ma.

                  Przycisk, który nic nie robi, jest gorszy niż jego brak:
                  wygląda jak droga. Adres wpisuje się więc ręcznie, i tak jest
                  napisane.
                */}
                <TextRow
                  label="Adres obrazu"
                  value={layer.url}
                  hint="Wklej adres obrazu. Wgrywanie plików tła czeka na składnicę galerii."
