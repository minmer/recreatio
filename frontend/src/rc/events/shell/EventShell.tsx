...
/**
 * Die Schichten eines Teils.
 *
 * Sie stehen im rc-Modell unter `layers` IM `configJson`, nicht in einer
 * eigenen Spalte. `parseLayers` will aber weiterhin eine Zeichenfolge — also
 * wird der Zweig herausgeschnitten und wieder zu einer gemacht, statt den
 * uebernommenen Leser umzuschreiben.
 *
 * Kaputtes JSON kostet die Farben, nicht die Seite: ohne Hintergrund ist sie
 * schlichter, ohne Inhalt waere sie weg.
 */
function layersJsonOf(configJson: string | null | undefined): string | null {
  if (configJson === null || configJson === undefined || configJson === '') return null;
  try {
    const parsed: unknown = JSON.parse(configJson);
    if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) return null;
    const layers = (parsed as Record<string, unknown>).layers;
    return layers === undefined ? null : JSON.stringify(layers);
  } catch { return null; }
}
...
export function EventShell({
  ...
  const labelOf = (part: (typeof parts)[number]): string => {
    const named = (part.menuLabel ?? '').trim();
    if (named !== '') return named;
    const titled = (part.title ?? '').trim();
    return titled === '' ? part.kind : titled;
  };

  const anchorIndex = new Map(parts.map((part, index) => [partAnchor(labelOf(part)), index]));
          ...
          <nav className="ev-pages" aria-label="Strony wydarzenia">
            {availablePages.map((entry) => {
              ...
              return pageHref ? (
                ...
                <button key={entry.slug} type="button" className={className} onClick={() => onSelectPage?.(entry.slug)}>
                  {label}
                </button>
              );
            })}
        ...
        <nav className="ev-parts" aria-label="Sekcje strony">
          {parts.map((part, index) => {
            ...
            return partHref ? (
              ...
              <button key={part.partId} type="button" className={className} onClick={() => scroll.scrollToSlide(index)}>
                {labelOf(part)}
              </button>
            );
          })}
              ...
              >
                ...
                <div className="ev-content-layer" style={{ height: `${slide.height}px` }}>
                  ...
                  >
                    ...
                    {module ? (
                      <module.Renderer
                        configJson={part.configJson ?? null}
                        ctx={{
                          collectionSlug: site.collectionSlug,
                          siteSlug: site.slug,
                          pageSlug: page.slug,
